...
namespace GMABot.Slash_Commands.Commands.Piracy.Clients
{
    ...
    public class Piracy1337xClient : IPiracyClient
    {
        ...
        public readonly string[] specialCategories = { "xxx", "tv" };
        ...
        public async Task<Torrent[]> GetTopTorrents(string searchText, string? category, int size)
        {
            ...
            return await FillTorrentsList(category, size, queryText);
        }

        private async Task<Torrent[]> FillTorrentsList(string? category, int size, string queryText)
        {
            ...
            while (torrents.Count < size)
            {
                ...
                try
                {
                    torrentPage = await GetTorrentsPage(queryText, category, page++);
                } catch (Exception)
                {
        ...
        public async Task<Torrent[]> GetTorrentsPage(string searchText, string? category, int page)
        {
            ...
            if (documentNode != null && documentNode.InnerText.Contains("No results were returned. Please refine your search."))
                throw new Exception();
